...
export async function generateQuote({
  ...
  if (avatarUrl) {
    try {
      ...
      const scale = Math.max(AVATAR_W / img.width, CANVAS_H / img.height);
      const drawW = img.width * scale;
      const drawH = img.height * scale;
      const drawX = (AVATAR_W - drawW) / 2;
      const drawY = (CANVAS_H - drawH) / 2;
      ctx.save();
      ctx.beginPath();
      ctx.rect(0, 0, AVATAR_W, CANVAS_H);
      ctx.clip();
  ...
  let y = Math.max(
    fontSize + 30,
    (CANVAS_H - (lines.length * lineHeight + ATTR_H)) / 2 + fontSize,
  );
